...
use super::rust::{f32_token, f64_token, post_transform_expr};
...
pub fn generate(forest: &Forest) -> Result<String> {
    ...
    if let PostTransform::Softmax { n_classes } = &forest.post_transform {
        ...
        return Ok(build_softmax(forest, &data_and_walker, n_trees, n_classes).to_string());
    }
    ...
    let post = post_transform_expr(&forest.post_transform);
...
mod tests {
    ...
    use crate::backends::rust::{
        generate_with_layout, resolve_layout, Layout, ARRAY_LAYOUT_NODE_THRESHOLD,
    };
