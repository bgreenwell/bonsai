...
// package root (two directories up from src/bin/).  include! (rather than a
// #[path] mod) keeps rustfmt from failing when the generated file is absent.
mod model_generated {
    include!("../../model_generated.rs");
}
...
fn main() -> anyhow::Result<()> {
    ...
    let mut df = if cli.input.extension().is_some_and(|ext| ext == "parquet") || cli.input.is_dir()
    {
        LazyFrame::scan_parquet(&input_str, ScanArgsParquet::default())?.collect()?
    } else {
    ...
    let feature_chunks: Vec<&ChunkedArray<Float32Type>> =
        feature_series.iter().map(|s| s.f32().unwrap()).collect();
    ...
    let scores: Vec<f32> = row_indices
        .par_chunks(chunk_size)
        .flat_map(|indices| {
            let n_rows = indices.len();
            let mut batch_features = vec![0.0f32; n_rows * cols];
            let mut batch_out = vec![0.0f32; n_rows * n_classes];

            for (i, &idx) in indices.iter().enumerate() {
                for (j, chunk) in feature_chunks.iter().enumerate() {
                    batch_features[i * cols + j] = chunk.get(idx).unwrap_or(f32::NAN);
                }
            }

            model.predict_batch(&batch_features, cols, &mut batch_out);
            batch_out
        })
        .collect();
    ...
    if n_classes == 1 {
        ...
        // For multiclass, create one column per class: "proba_0", "proba_1", ...
        // We need to de-interleave the flattened 'scores' vector.
        for c in 0..n_classes {
            let col_name = format!("proba_{}", c);
            let class_scores: Vec<f32> =
                scores.iter().skip(c).step_by(n_classes).copied().collect();
            let score_series = Series::new(&col_name, class_scores);
            df.with_column(score_series)?;
        }
    }
